...
10. **A cancelled delivery is not a deleted delivery** (slug: cancelled-delivery-leak) — `orderRepo.updateOrder`'s Delivery→Pickup cascade (#317) sets the linked `deliveries` row's `Status` to `Cancelled`; it does NOT soft-delete it (`deleted_at` stays `NULL`). Every read path that joins an order to `deliveryByOrderId`/`_delivery` (list enrichment, `GET /:id`, analytics, dashboard Today tab) therefore STILL finds that row — it must explicitly gate on the ORDER's current `Delivery Type`, not merely on "a delivery record exists." Reading pitfall #2's "prefer the delivery sub-record" without this extra gate resurrects a stale fee/address/driver from an order that is no longer a delivery. Any new fee/address/driver read from `deliveries` must check `order['Delivery Type'] === 'Delivery'` first (see `GET /:id` and the now-fixed `GET /` in `backend/src/routes/orders.js` for the pattern), AND the cascade itself must blank the delivery's own fee/address/recipient/driver fields (not just flip `Status`) plus the order's own redundant `Delivery Fee` column, so an ungated reader sees blanks instead of stale data. This was the root cause of #554 (fee + address survived a Delivery→Pickup conversion, even after refresh); fixed by gating `backend/src/routes/orders.js` `GET /` + blanking the extra fields in `orderRepo.updateOrder`; regression-locked by `backend/src/__tests__/orders.pickupConversion.integration.test.js`. Re-converting Pickup→Delivery after this does NOT revive the row (it stays Cancelled, `convertToDelivery`'s existing-check doesn't look at `status`) — a known, still-open follow-up gap, not something #554 introduced.
