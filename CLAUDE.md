...
2. **Delivery fee lives on the delivery record** (slug: delivery-fee-source) — `detail.delivery['Delivery Fee']`, NOT `detail['Delivery Fee']`. The order-level field is written once, at creation and at Pickup→Delivery conversion, and **nothing re-syncs it** — every fee editor PATCHes the delivery record alone. So it is stale from the first edit onward. Never inline the read: call **`resolveDeliveryFee`** — `backend/src/utils/deliveryGate.js` on the server, its same-named client mirror `packages/shared/utils/orderTotals.js` in the apps. It folds in both rules at once: a Pickup order owes nothing (pitfall #10 — its delivery row is only *cancelled*, fee intact), and within a Delivery order the delivery record wins **including when its fee is null or 0**, because a cleared fee means free delivery, not "fall back to the order's old value". A `||` chain gets that second rule wrong twice over. History: four backend readers and one dashboard reader had the precedence inverted (`order fee || delivery fee`), so an order set to 1000 zł with free delivery kept totalling 1035 zł — the list endpoint re-derived the fee and showed 1000 while the detail endpoint showed 1035, and no amount of reopening the order changed it (#644, prod order 202608-002). One of the five was the Unpaid→Paid backfill in `routes/orders.js`, i.e. it recorded a payment of the wrong amount. **`Final Price` is server-computed**, so a screen that PATCHes the fee and merges the response locally still shows the old total until it refetches — all three order-detail hosts now refetch on a `Delivery Fee` patch rather than recomputing the formula client-side. Driver Payout / Taxi Cost are the studio's COST of delivering (ADR-0019) and never belong in this number. Regression-locked by `backend/src/__tests__/orders.deliveryFeeClearRecalc.integration.test.js`, `packages/shared/test/orderTotals.test.js`, and `tests/e2e/order-total-delivery-fee.spec.js` (the click-through is what proves the third layer — the other two passed while the screen was frozen).
...
11. **A Wix mirror must only fire on a Wix-side CHANGE** (slug: wix-mirror-clobber) — `runPull` re-reads the whole Wix catalog and writes fields back into `product_config`. A mirror that overwrites unconditionally destroys any local edit Push has not landed yet: Wix reports the pre-Push value (either the Push silently lost the write, or Wix's read path lagged), Pull stamps it over the owner's value, and since `product_config` has no field history and price edits are not audit-logged, her intent is gone with no trace. Price now mirrors ONLY when Wix's own value moved since the previous Pull, compared against `product_config.wix_price_seen` (ADR-0020) — the same job `localNameOwned`/ADR-0008 does for Product Name. **Do not "fix" this with a time window.** That was the original proposal on #428 and prod `sync_log` refutes it: the 7 observed push→clobber events (2026-06-23 → 2026-07-22) had gaps from 49 seconds to 10.4 hours, so no N separates "stale echo of our Push" from "genuine Wix edit"; only "did Wix's value change" does. `wix_price_seen` is written by `runPull` alone and is deliberately NOT in `productConfigRepo`'s `EDITABLE_FIELD_MAP` — a route that could set it could forge the baseline. NULL = no baseline yet: record it, skip the mirror for that row, no backfill needed. Divergence is reported, never silently dropped (`stats.pricesNotOnWix` → `sync_log.prices_not_on_wix` → amber toast in both apps) — this bug survived two months precisely because `sync_log` recorded counts but never divergence. History: the price-push race that made Pushes silently fail was fixed separately in #572 (per-product PATCH batching) — that removed the dominant CAUSE, this removed the data-loss AMPLIFIER; both were needed. Regression-locked by `backend/src/__tests__/wixProductSync.pullPriceGuard.test.js`. **The sibling `Description` mirror in the same loop still clobbers unconditionally** — same bug class, not yet fixed.
...
11. **A Variety is resolved, never invented by accident** (slug: variety-identity-door) — `POST /api/stock` is the **single create-a-flower door**; seven screens across the three apps funnel into it (both receive forms, both new-order wizards, both order-detail "+ Add new" blocks, and the shared `createBouquetDemand`). It **matches before it creates**: `stockRepo.findVarietyMatch` resolves the posted identity (Variety 4-tuple when a Type is given, else the display name) case- and whitespace-insensitively, and the route returns that existing card with **200** instead of minting a duplicate with 201. Creating a genuinely new Variety requires an explicit `newVariety: true`. This is safe to do silently because every caller posts `quantity: 0` and logs the real movement separately against `res.data.id`. Identity stays strict and null-aware otherwise (ADR-0006): a blank Colour matches only a blank Colour, and a **dated Batch is never a match** — `Peony Pink (24.Jul.)` is one delivery, not the Variety's canonical card (see pitfall `batch-variety-attrs`); a caller posting a dated name is asking for that batch row and is left alone. Why it bites: the door's only rule used to be "displayName is not empty", so `Pink Peonies` landed beside `Peony / Pink` with Type literally `Pink Peonies`, and `peony` beside `Peony` — stock then splits across two cards and neither shows the true count (#562, the same shape as #319 and #558). **Never add a create-a-flower path that bypasses this door**, and never compare two flowers with a bare `===` on a name — use `sameVariety`/`normaliseIdentityValue` from `backend/src/utils/varietyIdentity.js`. Regression-locked by `backend/src/__tests__/stock.createMatchesExisting.integration.test.js` + `varietyIdentity.test.js`.
  ...
  **Purchase-order lines resolve too (#607) — the last three server-side create paths.** Everything above closes a path a *person* types into; these three were the server itself, and they were the widest, because `stockRepo.create` falls back to the **display name** for a missing Type. Composing a line with a name and no Type created a flower typed `Pink Peonies`; evaluation's 4-tuple branch matched through `stockRepo.list` (which also matches dated Batches) and created on a miss with no confirmation at all; evaluation's legacy name-only branch turned `Roses red 50` into a Type. All three now go through `stockRepo.findVarietyMatch` and **refuse rather than invent** — `resolveLineStockItem` is the named seam. **Creating a Variety at evaluation additionally requires a Type**: a name is not a classification, so a name-only line is a line error for the florist to classify, confirmation or not. The confirmation itself is now a COLUMN (`stock_order_lines.new_variety`, migration 0025) because composition, send and evaluation are days apart and evaluation is where the card is created; it is cleared the moment the line links to an existing card. **Where the question gets asked matters:** a complete line POSTed to `/stock-orders` or `/:id/lines` is refused immediately, but a Draft line built up field by field through PATCH stays permissive — blocking each intermediate edit fights the owner mid-typing — and **`/send` is that path's gate** (it also links any line that resolves on the way through). `PoLineForm` mirrors the split: a LINKED line keeps ADR-0016's blocking prompt (moving a line off its flower is destructive), a never-linked line applies the edit and leaves a standing amber notice until answered. The three 409s are returned by the routes DIRECTLY, never thrown — `errorHandler` rewrites every message to "Internal server error" in production and drops `code`, which is the only signal the editors can act on.
...
4b. **UI click-through** (any change a person interacts with — a form, a button, an inline editor):
   ...
   - **Write a spec for the specific behaviour you changed**, seeding only the data that behaviour needs (`tests/e2e/helpers/seed.js`) — don't widen the shared fixture. See `tests/e2e/stock-order-line-form.spec.js` as the model, and `tests/e2e/bouquet-flower-form.spec.js` for a behaviour that has to be proved on TWO hosts (a `dense` one and a non-dense one) because the bug lived in what each host passed down.
   - **The canonical fixture is thinner than it looks.** Its ten Stock rows carry NO Variety attrs, so every Y-model catalog collapses them into a single unnamed group — you cannot pick "Red Rose" by name from the wizard. Its three orders are dated 2026-04-28, which the dashboard Orders tab filters out by default (month-start → today). Seed a classified Variety (`seedVariety`) and a today-dated order (`seedOrder`) instead of leaning on the fixture; that is what `florist-order-creation.spec.js` had to be repaired to do (#609).
   ...
   - **Constraints that will otherwise waste an hour** (all learned the hard way, 2026-07-30):
     ...
     - Picking a customer in Step 1 of the new-order wizard does NOT advance it — the step then offers the optional key-person field and its own Continue (`wizard-next`, the same testid the page-level Next carries for steps 1–2).
