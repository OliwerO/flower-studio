...
export default function FinancialTab({ onNavigate }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Supplier Scorecard (with waste data) ── */}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-ios-tertiary border-b border-gray-100">
                  ...
                  <th className="text-right px-3 py-2 font-medium">{t.wasteLoss || 'Waste'}</th>
                  <th className="text-right px-3 py-2 font-medium">{t.wastePercent}</th>
                </tr>
              </thead>
              <tbody>
                {data.supplierScorecard.map(s => (
                  <tr key={s.supplier} className="border-b border-gray-50">
                    ...
                    <td className="px-3 py-2 text-right font-medium text-brand-700">{(s.totalSpend || 0).toFixed(0)} {t.zl}</td>
                    <td className="px-3 py-2 text-right text-ios-secondary">{(s.avgPricePerUnit || 0).toFixed(2)} {t.zl}</td>
                    <td className="px-3 py-2 text-right">
                      {(s.wasteCost || 0) > 0 ? (
                        <span className="text-ios-red font-medium">{s.wasteQty} ({s.wasteCost.toFixed(0)} {t.zl})</span>
                      ) : (
                        <span className="text-ios-tertiary">—</span>
                      )}
                    </td>
                    <td className="px-3 py-2 text-right">
                      {(s.wastePercent || 0) > 0 ? (
                        <span className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${
                          s.wastePercent > 15 ? 'bg-red-100 text-red-700' :
                          s.wastePercent > 5 ? 'bg-yellow-100 text-yellow-700' :
