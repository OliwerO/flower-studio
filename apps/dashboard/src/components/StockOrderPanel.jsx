              ...
              {lines.map(line => {
                ...
                <div key={line._idx} className="flex items-center gap-2 px-3 py-2 border-t border-gray-100">
                  <div className="flex-1 min-w-0">
                    {line.stockItemId ? (
                      <span className="text-sm font-medium text-ios-label">{line.flowerName}</span>
                    ) : (
                      <StockSearchInput
                        stock={stock}
                        value={line.flowerName}
                        onChange={val => updateFormLine(line._idx, { flowerName: val })}
...
// Inline stock search dropdown for adding new lines
  ...
  return (
    <div className="relative">
      ...
      {open && filtered.length > 0 && (
        <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-40 overflow-y-auto">
          {filtered.map(s => (
            <button
              key={s.id}
              type="button"
