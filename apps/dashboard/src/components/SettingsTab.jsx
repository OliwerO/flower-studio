...
// Manages all three category types: permanent (always visible), seasonal (date-based),
// and auto (system-generated like "Available Today"). All support multilingual translations.
function StorefrontCategoriesSection({ config: cfg, onUpdate }) {
  const sc = cfg.storefrontCategories || {};
  // Only one editor open at a time across all category types
  const [editingType, setEditingType] = useState(null); // 'permanent' | 'seasonal' | 'auto'
  ...
  // Ensure permanent/auto are object arrays (handle legacy string arrays from old configs)
  ...
  // Shared translation form used by all category types
  ...
  function renderCategoryRow(cat, type, i, extra) {
    return (
      <div key={i} className={`flex items-center gap-3 px-3 py-2 rounded-xl text-sm ${
        extra?.highlight ? 'bg-green-50 border border-green-200' : 'bg-gray-50 border border-gray-100'
      }`}>
    ...
    <Section title={t.sfCategories}>
      ...
      <div className="py-3 border-b border-gray-100">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            onClick={() => startEdit('permanent', 'new')}
            className="text-xs text-brand-600 font-medium hover:bg-brand-50 px-2 py-1 rounded-lg"
          >+ {t.addItem}</button>
        </div>
        <div className="space-y-1.5">
          {permanentList.map((p, i) => renderCategoryRow(p, 'permanent', i))}
        </div>
      ...
      <div className="py-3 border-b border-gray-100">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            onClick={() => startEdit('seasonal', 'new')}
            className="text-xs text-brand-600 font-medium hover:bg-brand-50 px-2 py-1 rounded-lg"
          >+ {t.addItem}</button>
        </div>
        <div className="space-y-1.5">
      ...
      <div className="py-3 border-b border-gray-100">
        ...
        <div className="space-y-1.5">
          {autoList.map((a, i) => renderCategoryRow(a, 'auto', i))}
        </div>
...
// ── Rate type names editor ──
function RateTypesEditor({ types, onSave }) {
  const [editing, setEditing] = useState(false);
...
// ── Per-florist, per-rate-type hourly rate editor ──
function FloristRatesEditor({ names, rateTypes, rates, onSave }) {
  const [editing, setEditing] = useState(false);
  ...
  return (
    <div className="space-y-4">
      ...
      <Section title={t.settingsLists}>
        ...
        />
        <RateTypesEditor
          types={config.rateTypes || ['Standard', 'Wedding', 'Holidays']}
          onSave={v => updateConfig({ rateTypes: v })}
        />
        <FloristRatesEditor
          names={config.floristNames || []}
          rateTypes={config.rateTypes || ['Standard', 'Wedding', 'Holidays']}
