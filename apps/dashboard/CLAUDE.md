...
| Explorer | ExplorerTab.jsx | **Owner-only linked-record grid** (ADR-0010, PRD #485) — a read-only 2nd front-end on the `query_records` engine Ask Blossom uses. Pick an entity → data-driven grid (columns from `GET /explorer/schema`); click-header sort + per-column filter popovers (shared `ColumnFilterPopover`, contains/exact/range by type — ALL server-side via the engine); a **Group by** select → count summary; **drill** buttons per row follow a relationship as a fresh seeded single-hop query (breadcrumb + Назад); rows deep-link into the real edit screens (orders/customers → `onNavigate`); **saved views** (`/explorer/views`) + **CSV export**. **Deep-join (v2, ADR-0011):** a "Связи/Deep-join" toggle switches to chain mode — a chain-builder strip (start entity → hop chips → "add related" menu, remove-last) builds a bounded edge path (`spec.chain`, max 4 hops) that the engine flattens into one denormalized grid (hop-prefixed "Entity · Field" columns, read via `chainCellValue`), with a fan-out warning on one-to-many hops; sort/filter/drill/group-by are off in chain mode (follow-ups). All chain logic in shared `explorerSpec` (`resolveChainColumns`/`availableChainEdges`/`chainRowsToCsv`/…). Safe by construction: only emits the same validated declarative spec (allow-list + row cap), never raw SQL, never writes. All spec/column/CSV logic in shared `explorerSpec` util + `useExplorerQuery` hook (unit-tested); Playwright rehearsal at `tests/e2e/explorer-ui.spec.js`. **Dashboard-only** — a documented exception to florist↔dashboard parity (owner exploration tool, not a shared operational feature). Lazy-loaded; mounted with `key={filterKey}` so an Ask Blossom "Open in Explorer ▸" handoff (`navigateTo({tab:'explorer', filter:{explorerSpec}})`) remounts it with the handed spec. |
