...
| Stock | StockTab.jsx | Inventory management with optimistic qty adjustments, write-off, visibility toggles, stock receive form. Uses `getEffectiveStock` from shared. Y-model collapsed Variety list (TypeGroupHeader + VarietyListItem + BatchTracePanel inline from shared). **Flat table** (BatchArrivalList): Available is **demand-aware** (#533 follow-up) — net (physical − committed) with a `· N committed · dd.MM` hint, so fully-promised stems never read as free; tapping a row opens the **Variety trace** (`fetchVarietyUsage` → `VarietyTracePanel`, same endpoint as the by-Variety view — a tier-scoped trail produced balances matching neither physical nor net). The "In stock" filter passes `hideEmpty` so 0-qty sell tiers inside a surviving Variety are dropped too (not just wholly-empty Varieties) — kills the legacy-fragment "0 available" duplicate rows; tiers with committed demand stay visible. Merged Batch tiers in the By-Variety expansion show their newest receive date **as the chip itself** (no anonymous "Batch"). The On-hand/In-premade bucket line drops the empty Planned column when no visible Variety has demand (numbers sit adjacent, still column-aligned). Tapping an order inside a Variety trace opens the shared `OrderQuickViewModal` popup instead of switching to the Orders tab (keeps you on the trace). **Flat table per-column filters (E1):** each header carries a shared `ColumnFilterPopover` funnel (hover-reveal, like OrdersTab) driving the shared `stockFilters` util (Type/Variety/Supplier contains, Available/Cost/Sell/Markup ranges, Arrived date range) — all client-side over the loaded set; a `Filters (n) · Reset` bar shows when active. **Totals footer (E2)** sums item count / qty / cost value / sell value over the visible (filtered) rows. |
...
| Explorer | ExplorerTab.jsx | **Owner-only linked-record grid** (ADR-0010, PRD #485) — a read-only 2nd front-end on the `query_records` engine Ask Blossom uses. Pick an entity → data-driven grid (columns from `GET /explorer/schema`); click-header sort + per-column filter popovers (shared `ColumnFilterPopover`, contains/exact/range by type — ALL server-side via the engine); a **Group by** select → count summary; **drill** buttons per row follow a relationship as a fresh seeded single-hop query (breadcrumb + Назад); rows deep-link into the real edit screens (orders/customers → `onNavigate`); **saved views** (`/explorer/views`) + **CSV export**. **Unified grid (v2 Wave 3, #504):** no mode toggle — every grid is a **path** (0 hops = plain). An always-present **path builder** (start table → "add related" menu → hop chips, remove-last) extends it into a bounded edge chain (`spec.chain`, max 4 hops, ADR-0011) the engine flattens into one denormalized grid (hop-prefixed "Entity · Field" columns via `chainCellValue`, fan-out warning on one-to-many hops). A **column picker** ("Columns (N)") ticks columns on/off across the path; selection persists in `spec.columns` (qualified `entity.field`) → saved views + assistant handoff. Grids open on **curated `primary` columns** (from the descriptor), not every field. **Sort + per-column filter work on any grid — 0-hop AND multi-hop** (each column emits its qualified `entity.field` ref, which the engine resolves via `buildScope`/`resolveInScope`; re-runs server-side, respecting the row cap). Group-by/Open-deep-link stay 0-hop-only. Ask Blossom can hand off a curated `chain`+`columns` view. All logic in shared `explorerSpec` (`visibleColumns`/`toggleColumn`/`columnId`/`resolveChainColumns`/`availableChainEdges`/`chainRowsToCsv`/…). Safe by construction: only emits the same validated declarative spec (allow-list + row cap), never raw SQL, never writes. All spec/column/CSV logic in shared `explorerSpec` util + `useExplorerQuery` hook (unit-tested); Playwright rehearsal at `tests/e2e/explorer-ui.spec.js`. **Dashboard-only** — a documented exception to florist↔dashboard parity (owner exploration tool, not a shared operational feature). Lazy-loaded; mounted with `key={filterKey}` so an Ask Blossom "Open in Explorer ▸" handoff (`navigateTo({tab:'explorer', filter:{explorerSpec}})`) remounts it with the handed spec. |
