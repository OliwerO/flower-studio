...
| OrderListPage | /orders | all | Today's worklist — active/completed tabs, status filters, owner dashboard alerts. A compact **fulfilment-date sort pill** next to the Filters button (#247) cycles default (existing per-view sort, unchanged) → ascending (earliest first) → descending (latest first) — same `Delivery Date`/`Required By` field precedence as the dashboard `OrdersTab.jsx` column sort (PR #442), session-persisted via the same `#338` sessionStorage pattern as the rest of the list's filter/view state. |
...
| Component | Purpose |
|-----------|---------|
| OrderCard.jsx | Expandable order card — inline status/payment editing, bouquet editor, delivery fields. Largest component (1300+ L — split candidate; uses `OrderCardSummary.jsx`). Adds a flower via the shared `BouquetFlowerForm`. Order Cancellation flows through `useOrderTerminationFlow` + `OrderTerminationConfirm` from shared (no inline handler). |
