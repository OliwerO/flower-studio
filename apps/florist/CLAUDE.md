...
| StockPanelPage | /stock | all | Inventory view with search, sort, filter, adjust, write-off, receive. Under `STOCK_Y_MODEL` a **Flat ⇄ By type** layout toggle (persisted `blossom-florist-stock-flat`, **default Flat/ungrouped**): Flat = one VarietyListItem row per Variety; a compact **sort pill cycles A–Z (default) → Longest in stock → Stock level** (persisted `blossom-florist-stock-sort`). By type = the TypeGroupHeader-grouped list. Both share `renderVariety`. Rows pass `showHeaderTrace={false}` — no per-row trace icon; trace opens via tap-to-expand → **Trace**. Negative/Low/Slow pills + search + hide-zero apply in both. Legacy flat StockItem list when the flag is off. |
