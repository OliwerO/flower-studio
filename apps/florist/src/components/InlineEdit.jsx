...
// Ported verbatim from apps/dashboard/src/components/InlineEdit.jsx so the
// florist Customer tab's editable fields behave the same as the dashboard's.
...
export default function InlineEdit({
  ...
  if (multiline) {
    return (
      <textarea
        ...
        onBlur={commit}
        rows={2}
        className="w-full px-2 py-1 text-sm border border-gray-300 rounded resize-none focus:border-brand-500 focus:outline-none"
      />
    );
  }
  ...
  return (
    <input
      ...
      className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:border-brand-500 focus:outline-none"
    />
  );
}
