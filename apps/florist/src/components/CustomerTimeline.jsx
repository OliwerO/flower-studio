...
// `raw`. For app orders, a "View full details" button calls `onNavigate`
// so the parent page can route to the full order detail — on mobile that's
// `/orders/:id`, matching the dashboard behaviour of cross-tab navigation
// with an orderId filter.
//
// Ported 2026-04-22 from apps/dashboard/src/components/CustomerTimeline.jsx.
// The only interface difference is what `onNavigate` does: dashboard switches
// tabs, florist pushes a route. The component doesn't care — it just hands
// the orderId up. Keeps the two implementations trivially mergeable later.
...
// Status → pill color. Same palette as florist OrderCard so the owner reads
// the same colors everywhere. Unknown statuses fall back to gray.
const STATUS_COLORS = {
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-semibold text-ios-tertiary uppercase tracking-wide">
          {t.timeline || 'Timeline'} ({orders.length})
        </p>
      ...
      {filtered.length === 0 ? (
        <p className="text-sm text-ios-tertiary py-4 text-center">{t.noResults || 'No results'}</p>
      ) : (
    ...
    <div className="border-b border-white/30 last:border-b-0">
      ...
      >
        ...
        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded uppercase tracking-wide shrink-0 ${
          ...
          {isLegacy ? (t.legacyOrder || 'Legacy') : (t.appOrder || 'App')}
        </span>
        {fulfilmentIcon && (
        ...
        <span className="text-sm font-medium text-ios-label shrink-0 w-20 text-right">
          {amount ? `${amount.toFixed(0)} ${t.zl || 'zł'}` : '—'}
        </span>
      </button>
    ...
    <div className="bg-white/60 px-4 py-3 border-t border-white/40">
      ...
      {source === 'app' && onNavigate && (
        <div className="mt-3 pt-2 border-t border-white/40">
          ...
          >
            {t.openInOrdersTab || 'Open in Orders'} →
          </button>
        </div>
      )}
    </div>
