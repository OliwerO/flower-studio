...
## 2026-07-06 — fix(stock): Y-model-safe PO→stock — substitute double-book + missing-Type 500s

Owner report (PO-20260705-1): flowers ordered but not delivered were sourced from an alternative supplier; after florist evaluation, **both** the original AND the substitute were booked into stock, creating phantom original batches (Dahlia Pink +10, Peony +14, Lisianthus +10) that never arrived. Prod data corrected out-of-band (3 phantom batches soft-deleted, 3 phantom `stock_purchases` voided, Dahlia Coral relabeled Type=Dahlia/Colour=Coral).

- **Double-book guard** (`routes/stockOrders.js` `/evaluate`): a line with `Driver Status = "Not Found"` (the original did not arrive — it was substituted) now forces primary `accepted`/`writeOff` to **0**, regardless of the submitted quantity. Authoritative server invariant; the phantom is structurally impossible. Florist UI mirrors it: `StockEvaluationPage` hides the primary accept row + defaults accepted 0 on Not Found; `ShoppingSupportPage` zeroes `Quantity Found` when a line is marked Not Found.
- **Substitute Type/Colour picker**: `StockEvaluationPage` substitute classification is now a `<datalist>` seeded from existing Varieties (was free-text → a substitute once got `Type="Dahlia Pink"`). `GET /stock/distinct/:column` widened to **florist** (read-only) to feed it.
- **`type_name` NOT NULL safety net**: `stockRepo.create` defaults `type_name` from the base Display Name when missing. Closes 5 create paths (`stockOrders.js` :54/:614/:685/:867, `stockPurchases.js` :44) that could hit `null value in column "type_name"` → 500 → empty toast / PO stuck in Eval Error. **The test-harness pglite schema lacks this NOT NULL constraint that prod has, so CI never caught the class** — consider hardening the harness.
- Bonus: shopping-view money inputs accept pl-locale comma decimals and no longer strip in-progress decimals on keystroke.
- Backend-only logic + florist UI; no schema migration. Regression: `stockOrders.substituteDoubleBook.integration.test.js`. Verified: backend vitest **933**, E2E **253**, florist build green.
- **Still open (W2, urgent — actively 500ing):** the `stock_demand_variety_date_idx` dup-key collision documented under #508 below. Compounded by the cutover backfilling 62 active rows onto one shared date (`2026-06-29`); 13 variety-groups share a date, so order consumption driving a 2nd same-variety batch negative collides. Fix = route order-demand creation through `getOrCreateDemandEntry` (sum, don't duplicate) + re-date the legacy shared-date rows.
...
- The "create demand entry" flow (`useOrderEditing.createDemandEntry`) posts to `POST /stock` when a variety isn't in stock yet — peony-specific only because that variety wasn't matched to an existing row. `stockRepo.create` never set `date`, but prod PG enforces **`stock.date NOT NULL`** — the Y-model is **live on prod** (`STOCK_Y_MODEL=true` on service `flower-studio-backend`; the default-linked Railway service is `Postgres`, so a bare `railway variables` misleadingly shows no flag). So a dateless stock row is genuinely invalid; the undated INSERT was rejected → generic 500 → empty toast.
...
- **Follow-up (not this PR):** under the live Y-model this flow should route through `getOrCreateDemandEntry` (with the order's Required By) so it **sums into an existing dated Demand Entry** instead of creating a fresh plain item via `POST /stock`. As-is, two same-variety demand entries created the same day become two `qty<0` rows sharing `(variety, date)` and collide on the `stock_demand_variety_date_idx` unique index at the negative-qty update (source of the 2026-07-03 dup-key errors in prod logs). The `date=today` default unblocks creation but doesn't fix this duplication path.
