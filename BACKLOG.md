...
Phase 7 complete (2026-05-09). Postgres is the only datastore. Migration scripts archived under `docs/migration/`. Historical phase-by-phase notes remain below for reference.
...
10. **Phase 7 PR 2 — Retire Airtable** (split into PR 2a + PR 2b) — **DONE 2026-05-09**
    - **PR 2a (DONE 2026-05-09)** — migrated live Airtable bypasses to Postgres: Wix webhook (`processWixOrder` + `reprocessWixOrder`) writes via new `orderRepo.createWixOrder` and reads stock via `stockRepo`; `public.js` + `intake.js` stock reads via `stockRepo`; 7 `listByIds(TABLES.X, …)` callers in `routes/orders.js` + `services/orderService.js` replaced with new `orderRepo.getLinesByIds` / `getDeliveriesByIds`; `routes/test.js` `/test/reset` seeds Postgres directly from `airtable-test-base.json` (drops airtable-mock dependency).
    - **PR 2b (DONE 2026-05-09)** — deleted 7 source files (`airtable.js`, `airtable-real.js`, `airtable-mock.js`, `airtable-mock-formula.js`, `airtableSchema.js`, `config/airtable.js`, `utils/batchQuery.js`), dropped `airtable` npm dep, stripped `STOCK_BACKEND` / `ORDER_BACKEND` env flag logic + boot guard, stripped ~600 LOC dead fallback branches, deleted `mirrorAirtableOrder` + `runParityCheck`. Owner action: cancel Airtable subscription + remove `AIRTABLE_*` env vars from Railway.

**Phase 7 PR 2b checklist — cleanup items identified during PR 2a boundary review (2026-05-09):**
- [x] **Delete `mirrorAirtableOrder`** (`backend/src/repos/orderRepo.js:360-406`) — function is dead after PR 2a removed its only caller (T2). Delete alongside the `airtable.js` sweep in PR 2b to avoid keeping unreachable code.
- [x] **Remove stale dedup comment in `processWixOrder`** (`backend/src/services/wix.js:147`) — comment says "airtable mode falls back to the existing filterByFormula path"; that fallback branch in `orderRepo.findByWixOrderId` is now unreachable (airtable mode gone). Delete comment + dead branch together in PR 2b.
- [x] **Delete `backend/src/utils/batchQuery.js`** — PR 2a T6 migrated all live callers (`orders.js`, `orderService.js`) to `orderRepo.getLinesByIds` / `getDeliveriesByIds`. The utility is referenced only by `stockRepo.listByIds`'s airtable-mode fallback (dynamic import at `stockRepo.js:248`), which is unreachable in production but kept by PR 2a's "fallback branches stay" rule. Delete the utility together with the stockRepo airtable fallback in PR 2b's repo sweep.
- [x] **Replace `db.getById(TABLES.STOCK, ...)` at `routes/orders.js:~605`** — pre-existing call uses `airtable.js` directly. The comment claims "the route doesn't yet have a stockRepo.getById helper" but `stockRepo.getById` exists since Phase 3. Surfaced by PR 2a Phase B boundary review (out of scope for PR 2a since it's not a `listByIds` migration target). Replace with `stockRepo.getById` and drop the stale comment.

**Phase 7 PR 2c (future / unscheduled):**
- [ ] **Unit/integration test for `processWixOrder` / `reprocessWixOrder` orchestration** — E2E section 24 covers the happy-path replay end-to-end, but there is no dedicated unit test for: customer-match-on-replay (existing customer found vs. created), per-line `productConfigRepo.decrementQuantity` invocation, and duplicate-wixOrderId guard. Add a `wixProcessOrder.unit.test.js` (mock `orderRepo`, `customerRepo`, `productConfigRepo`) to cover these branches without needing the full harness.
...
- [x] **Report system — in-app bug/feature reporting** (2026-05-07, PRs #235–244) — Owner/Florist/Driver tap "Сообщить о проблеме" in any app; Claude Haiku asks clarifying questions in Russian until the report is complete; reporter sees a Russian preview and confirms; a structured English GitHub issue is created with `needs-triage` label. Screenshots attached via GitHub Contents API. Telegram bot (`FEEDBACK_BOT_TOKEN`) for owner: `/start <PIN>` to register, then any message starts a report session. GitHub issues webhook (`POST /webhook/github`) sends Telegram close notification when the issue is resolved. New: `feedback_reports` table (migration 0008), `feedbackService.js`, `feedbackTelegramBot.js`, `FeedbackModal` shared component, `POST /feedback/*` routes.
- [x] **Bouquet image upload + driver display** (2026-05-03) — owner + florist upload bouquet photos from clipboard or phone library on the florist app and dashboard; image stored in Wix Media + cached locally via new `productRepo` (Phase 6/7 migration-safe). Driver `DeliveryCard` renders the bouquet image (tap-to-zoom) above the address so the right physical bouquet is identifiable on pickup. New endpoints: `POST /products/:id/image` (florist+owner), `DELETE` (owner only) — orchestrated server-side via `wixMediaClient.js`. Shared `BouquetImageEditor` + `BouquetImageView` components in `@flower-studio/shared`. SSE event `product_image_changed` for instant cross-app updates. E2E Section 25 (15 assertions) covers upload + delete + role gates + MIME validation; harness intercepts Wix Media when `HARNESS_MOCK_WIX=1`. Bonus: fixed empty nav-pill bug (Tailwind `content` glob now scans `packages/shared`).
