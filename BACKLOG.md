...
10. **Phase 7 PR 2 — Retire Airtable** (split into PR 2a + PR 2b)
    - **PR 2a (DONE 2026-05-09)** — migrate live Airtable bypasses to Postgres: Wix webhook (`processWixOrder` + `reprocessWixOrder`) writes via new `orderRepo.createWixOrder` and reads stock via `stockRepo`; `public.js` + `intake.js` stock reads via `stockRepo`; 7 `listByIds(TABLES.X, …)` callers in `routes/orders.js` + `services/orderService.js` replaced with new `orderRepo.getLinesByIds` / `getDeliveriesByIds`; `routes/test.js` `/test/reset` seeds Postgres directly from `airtable-test-base.json` (drops airtable-mock dependency).
    - **PR 2b (NEXT — wait ≥48h after PR 2a deploy)** — delete `backend/src/services/airtable.js`, `airtable-real.js`, `airtable-mock.js`, `airtable-mock-formula.js`, `airtableSchema.js`, `config/airtable.js`, `utils/batchQuery.js`. Drop `airtable` npm dependency. Strip `STOCK_BACKEND` / `ORDER_BACKEND` env flag logic + boot guard from `index.js` / `orderRepo.js` / `stockRepo.js` / `orderService.js` (PG-only since cutover). Remove `tblMockStock*` etc. fixture data still needed only by airtable-mock. Final Airtable snapshot. Cancel subscription.

**Phase 7 PR 2b checklist — cleanup items identified during PR 2a boundary review (2026-05-09):**
- [ ] **Delete `mirrorAirtableOrder`** (`backend/src/repos/orderRepo.js:360-406`) — function is dead after PR 2a removed its only caller (T2). Delete alongside the `airtable.js` sweep in PR 2b to avoid keeping unreachable code.
- [ ] **Remove stale dedup comment in `processWixOrder`** (`backend/src/services/wix.js:147`) — comment says "airtable mode falls back to the existing filterByFormula path"; that fallback branch in `orderRepo.findByWixOrderId` is now unreachable (airtable mode gone). Delete comment + dead branch together in PR 2b.
- [ ] **Delete `backend/src/utils/batchQuery.js`** — PR 2a T6 migrated all live callers (`orders.js`, `orderService.js`) to `orderRepo.getLinesByIds` / `getDeliveriesByIds`. The utility is referenced only by `stockRepo.listByIds`'s airtable-mode fallback (dynamic import at `stockRepo.js:248`), which is unreachable in production but kept by PR 2a's "fallback branches stay" rule. Delete the utility together with the stockRepo airtable fallback in PR 2b's repo sweep.
- [ ] **Replace `db.getById(TABLES.STOCK, ...)` at `routes/orders.js:~605`** — pre-existing call uses `airtable.js` directly. The comment claims "the route doesn't yet have a stockRepo.getById helper" but `stockRepo.getById` exists since Phase 3. Surfaced by PR 2a Phase B boundary review (out of scope for PR 2a since it's not a `listByIds` migration target). Replace with `stockRepo.getById` and drop the stale comment.
