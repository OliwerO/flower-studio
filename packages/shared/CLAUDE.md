...
components/
  ...
  BalanceSparkline.jsx        → Shared step-chart balance trace (S7). Time-proportional X axis, inverted Y axis, always-on zero line (dashed), staircase path (hold→jump), Y/X axis labels, colour-coded event markers. Props: events, t, onOrderClick, asOf, **opening** (B2 — pre-record stock the running balance starts from, so post-cutover histories don't dive below zero and a write-off never sits on an empty shelf; renders an indigo `opening-marker`). Consumed by BatchTracePanel and VarietyTracePanel.
