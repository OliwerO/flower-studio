...
- `utils/driverPins.js` — `resolveDriverByPin(pin)`: resolves a PIN string to a driver name by scanning `PIN_DRIVER_*` env vars. `resolveFloristByPin(pin)`: resolves `PIN_FLORIST` → `'florist'`. Shared utility used by both the driver/florist registration bot and PIN auth.
