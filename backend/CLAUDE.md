...
| assistantTools/ | Assistant tool-pack registry. Each pack file is a read-only thin adapter over a canonical service/repo (`financePack` → `analyticsService.computeAnalytics`, `ordersPack` → `orderRepo.list`, `stockPack` → `stockRepo.list` + `stockLossRepo.list`). Adding a domain = add a pack file + import + push into `TOOLS` in `index.js`. Never inline SQL or recompute business logic here — delegate to the canonical service. Stock available qty = `Current Quantity` as-is (pitfall #8 — never subtract committed); shortfall = qty<0. |
