...
| explorer.js | POST /api/explorer/query; GET /api/explorer/schema (owner-only, `authorize('explorer')`) | **Explorer** — the read-only, owner-only second front-end on the `query_records` engine (ADR-0010). `POST /query` accepts a `query_records` spec, runs the SAME `queryRecordsHandler` (returns `{spec, matchedCount, truncated, rows}` or 400 `{error}`); `GET /schema` returns `explorerSchema.describeSchema()` (allow-listed entities/fields/drill-edges with Russian labels) so the Dashboard grid is data-driven. Thin controller — all validation/execution stays in `dataQueryPack.js`. Saved views (`saved_views` table + `savedViewRepo`) exist; their CRUD route + UI land in a later Explorer wave. |
