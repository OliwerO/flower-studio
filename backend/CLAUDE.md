...
| settings.js | GET/POST /settings, PUT /settings/driver-language (owner-only; sets a Driver's notification language ru/en/pl in `driver_telegram_chats`) | Thin HTTP layer for app config. State lives in `services/configService.js`. Re-exports its getters for backward compat — callers should migrate to importing from configService directly. |
...
- `utils/driverPins.js` — `resolveDriverByPin(pin)`: resolves a PIN string to a driver name by scanning `PIN_DRIVER_*` env vars. Shared utility used by both the driver registration bot and PIN auth.
