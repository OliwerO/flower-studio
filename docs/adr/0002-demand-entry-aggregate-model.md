...
> **Superseded by [ADR-0005](./0005-stock-y-model-dated-demand-entries.md) (2026-05-10).** The "at most one Demand Entry per variety" invariant is replaced by "at most one Demand Entry per (Variety, date)." Premade bouquets switch from Batch-decrement to reservation. Variety identity is the four-tuple defined in [ADR-0006](./0006-variety-four-tuple-identity.md).

When stems are added to an order but no Batch exists (or the owner explicitly declines to use an existing Batch), the system creates or deepens a single undated Stock Item — a Demand Entry — for that variety. All future orders drawing on the same variety share this one record; the quantity goes increasingly negative as more demand is committed. When a Stock Order arrives and is evaluated, a new Batch record is created for the arrived stems. If the Demand Entry for that variety has a negative quantity (pre-sold demand), the deficit is absorbed into the new Batch (received − deficit), and the Demand Entry is zeroed out. The Demand Entry record is kept (rather than renamed or deleted) so it remains as an audit marker of what was pre-sold against that Stock Order. Order lines continue to point at the Demand Entry record ID throughout.
