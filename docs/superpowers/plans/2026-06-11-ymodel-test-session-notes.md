...
## Session 2c — 2026-06-12 — Stock-panel netting (post-S1 review)
- **CR-38 — Drop the "N varieties · N stems short" summary from ShortfallSummary.** Owner: "nothing we will need." DONE in S1/S2 chain (`ShortfallSummary.jsx` header), matching the pending-panel summary removal (CR-33).
- **CR-39 — Shortfalls must net against IN-TIME pending arrivals (date-aware effective).** Today ShortfallSummary shows raw negative demand and ignores `/stock/pending-po`; PendingArrivals shows incoming separately. Owner wants the prod-style net coverage — BUT under Y-model coverage is **date-aware** (the owner's "might not be possible on batch when we don't have batches" insight). Worked examples (`y-model-guide`):
  - Peony Pink −7 **needed 15.06** + arrival +7 **on 16.06** → **late** → still a shortfall.
  - Lisianthus −12 **needed 18.06** + arrival +20 **on 16.06** → **in time** → covered (+8 surplus) → drop from shortfalls.
  - Ranunculus −5 **needed 20.06**, no PO → genuine shortfall.
  Prod nets a single aggregate `qty + incoming` (no dates on the X-model); Y-model can net per (Variety, needed-by vs arrival) instead.
  - **Decision needed (extends D3):** does an arrival AFTER the needed date count as coverage? Proposed: **No** — a late arrival does NOT clear the shortfall for that date (you can't fulfil the order on time), but show it as "covered late" (amber) rather than a hard red short, so the owner sees a PO already exists. An in-time arrival clears it (green/removed). Pure surplus shows under pending only.
  - **Scope:** this is the **effective-availability engine** (S3 / decision D3) surfacing in the **stock panel**, not just the picker. Build the shared date-aware `effective(variety, byDate)` calc once; ShortfallSummary + PendingArrivalsPanel + the picker all consume it. → fold into **S3** (or a dedicated "stock-panel effective" pass right after S2).
