...
## Wave 3 (RE-SCOPED) — readable unified Explorer (#504)  ✅ BUILT on `feat/explorer-v2-readable-columns`
From owner live use: deep-join unreadable + plain/deep-join toggle confusing. Merged into ONE path grid (0 hops = plain; "+ add related" extends), curated `primary` default columns, column picker (`spec.columns`), assistant curates columns via `open_explorer_view` (chain+columns). Removed navigate-drill + toggle. Follow-ups: sort/filter over multi-hop grid; then pivot.

## Wave 4 — pivot & measures (#499)  [own branch]
